...
#         llr_a = mem_alpha[stage_depth][mem_alpha_ptr[stage_depth] + i]
#         llr_b = mem_alpha[stage_depth][mem_alpha_ptr[stage_depth] + i + stage_size // 2]
#         mem_alpha[stage_depth - 1][mem_alpha_ptr[stage_depth - 1] + i] = min(llr_a, llr_b) * (1 if llr_a * llr_b >= 0 else -1)
#         if is_quantized:
#             mem_alpha[stage_depth - 1][mem_alpha_ptr[stage_depth - 1]] = min(max_value, mem_alpha[stage_depth - 1][mem_alpha_ptr[stage_depth - 1]])
#             mem_alpha[stage_depth - 1][mem_alpha_ptr[stage_depth - 1]] = max(min_value, mem_alpha[stage_depth - 1][mem_alpha_ptr[stage_depth - 1]])
...
#         if mem_beta[stage_depth - 1][mem_alpha_ptr[stage_depth] + i] == 0:
#             mem_alpha[stage_depth - 1][mem_alpha_ptr[stage_depth - 1] + i] = llr_b + llr_a
#         else:
#             mem_alpha[stage_depth - 1][mem_alpha_ptr[stage_depth - 1] + i] = llr_b - llr_a

#         if is_quantized:
#             mem_alpha[stage_depth - 1][i] = min(max_value, mem_alpha[stage_depth - 1][i])
#             mem_alpha[stage_depth - 1][i] = max(min_value, mem_alpha[stage_depth - 1][i])
...
#     mem_beta[0][mem_alpha_ptr[0]] = 0 if is_frozen else 1 if llr < 0 else 0
...
def dec_sc(vec_decoded, vec_dec_sch, mem_alpha, mem_beta, mem_alpha_ptr, mem_beta_ptr, vec_dec_sch_size, vec_dec_sch_depth, vec_polar_isfrozen, qbits_enable, quant_intl_max, quant_intl_min):
    ...
    for i in range(len(vec_dec_sch)):
        ...
        elif vec_dec_sch[i] == 'H':
            mem_beta[:, 0, mem_alpha_ptr[0]] = 0
            mem_alpha_ptr[0] += 1
        elif vec_dec_sch[i] == 'I':
